...
Vector3f Scene::castRay_noBVH(const Ray &ray, int depth) const
{
    if (depth > this->maxDepth) {
        return Vector3f(0.0,0.0,0.0);
    }    
                    ...
                    for (uint32_t i = 0; i < get_lights().size(); ++i)
                    {
                        ...
                        else
                        {
                            ...
                            for (const auto& obj : this->get_objects()) {
                                ...
                                if (mesh) {
                                    for (auto& tri : mesh->triangles) {
                                        Intersection inter = tri.getIntersection(shadowRay);
                                        if (inter.happened && inter.distance * inter.distance < lightDistance2) {
                                            inShadow = true;
                                            break;
                                        }
                                    }
